...
Artisan.prototype.register = function (target, caller) {
    ...
    glob.sync('*/index.js', { cwd: search })
        .map(function (p) {
            ...
            return path.relative(base, path.join(caller, p));
        })
 ...
 * @param {string} p
 */
Artisan.prototype.addProvider = function (p) {
    // Don't re-define providers
    var existing = _.find(this.providers, { relativePath: p });
    if (typeof existing !== 'undefined') {
        return;
    }
    ...
    var provider = new Provider(this, p, slugBase);
