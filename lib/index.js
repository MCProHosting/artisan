...
/**
 * Tries to load and return a module by name. Accepts a second argument
 * which can mock items directly.
 *
 * @param  {string} name
 * @param  {object} mocks
 * @return {*}
 */
Artisan.prototype.resolve = function (name, mocks) {
    // First, try to see if we have the module in the cache already.
    var cached = this.moduleCache[name];
    if (typeof cached !== 'undefined' && !mocks) {
        return cached;
    }

    // If we're loading things relatively and are inside a provider,
    // prepend that provider's slug to what we want to get.
    if (this.currentProvider && name.slice(0, 2) === './') {
    ...
    // will be thrown).
    return this.resolve(name, mocks);
};
